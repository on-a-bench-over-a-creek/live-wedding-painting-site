...
function packagesPage() {
  const packages = [["The Classic", "24x36", "Timeless and detailed piece in a classic style", "$850"], ["The Nostalgia Keepsake", "30x40", "The signature experience: a generous canvas, your chosen scene, and the whole room invited in.", "$1,200"], ["The Memory Montage", "36x48", "A larger-scale heirloom with extra details and time for the little moments.", "$1,850"]];
  return `<div class="page"><header class="page-header"><div class="container"><h1>Choose the shape of your story.</h1><p class="intro">Find what fits your dream event</p></div></header><section class="section"><div class="container"><div class="package-grid">${packages.map((item, index) => `<article class="package-card${index === 1 ? " featured" : ""}"><p class="eyebrow">Package 0${index + 1}</p><h3>${item[0]}</h3><p class="package-subtitle">${item[1]}</p><p>${item[2]}</p><p class="price">${item[3]}</p><ul>${index === 0 ? "<li>1 couple pose in full detail</li><li>5 collage elements of your choice</li><li>Guest interactions</li><li>Finished piece</li>" : index === 1 ? "<li>1 couple pose in full detail</li><li>2 colorful painted memories</li><li>5 collage elements of your choice</li><li>Guest interactions</li><li>Custom stickers</li><li>Finished piece</li>" : "<li>Live painting during your celebration</li><li>Archival materials</li><li>[Placeholder inclusion]</li>"}</ul>${button("Ask about this", "contact", index !== 1)}</article>`).join("")}</div></div></section></div>`;
}
