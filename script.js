...
function packagesPage() {
  const packages = [["The Classic", "A beautiful introduction to live painting for intimate celebrations.", "$1,800"], ["The Nostalgia Keepsake", "The signature experience: a generous canvas, your chosen scene, and the whole room invited in.", "$2,800"], ["The Memory Montage", "A larger-scale heirloom with extra details and time for the little moments.", "$3,800"]];
  return `<div class="page"><header class="page-header"><div class="container"><h1>Choose the shape of your story.</h1><p class="intro">Find what fits your dream event</p></div></header><section class="section"><div class="container"><div class="package-grid">${packages.map((item, index) => `<article class="package-card${index === 1 ? " featured" : ""}"><p class="eyebrow">Package 0${index + 1}</p><h3>${item[0]}</h3><p>${item[1]}</p><p class="price">${item[2]}</p><ul><li>Live painting during your celebration</li><li>Archival materials</li><li>[Placeholder inclusion]</li></ul>${button("Ask about this", "contact", index !== 1)}</article>`).join("")}</div></div></section></div>`;
}
